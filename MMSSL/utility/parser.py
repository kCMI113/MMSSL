...
def parse_args():
    ...
    parser.add_argument(
        "--layers", type=int, default=1, help="Number of feature graph conv layers"
    )
    parser.add_argument(
        "--mess_dropout",
        nargs="?",
    ...
    parser.add_argument(
        "--sparse", type=int, default=1, help="Sparse or dense adjacency matrix"
    )
    ...
    parser.add_argument(
        "--metapath_threshold", default=2, type=int, help="metapath_threshold"
    )
    ...
    parser.add_argument(
        "--gss_rate", default=0.85, type=float, help="gene_self_subgraph_rate"
    )
    parser.add_argument("--anchor_rate", default=0.75, type=float, help="anchor_rate")
    parser.add_argument(
        "--feat_reg_decay", default=1e-5, type=float, help="feat_reg_decay"
    )
    ...
    parser.add_argument(
        "--fake_gene_rate", default=0.0001, type=float, help="fake_gene_rate"
    )
    parser.add_argument(
        "--ID_layers", type=int, default=1, help="Number of item graph conv layers"
    )
    ...
    parser.add_argument(
        "--negrate", default=0.01, type=float, help="item_neg_sample_rate"
    )
    ...
    parser.add_argument(
        "--isload", default=False, type=bool, help="whether load model"
    )  #
    parser.add_argument(
        "--isJustTest", default=False, type=bool, help="whether load model"
    )
    parser.add_argument(
        "--loadModelPath",
        default="/home/ww/Code/work3/BSTRec/Model/retailrocket/for_meta_hidden_dim_dim__8_retailrocket_2021_07_10__18_35_32_lr_0.0003_reg_0.01_batch_size_1024_gnn_layer_[16,16,16].pth",
        type=str,
        help="loadModelPath",
    )
    ...
    parser.add_argument(
        "--data_path",
        nargs="?",
        default="/home/ww/Code/work5/MMSSL/data/",
    ...
    parser.add_argument(
        "--dataset",
        nargs="?",
        default="amazon_clothing_10_core",
    ...
    parser.add_argument(
        "--topk", type=int, default=10, help="K value of k-NN sparsification"
    )
    parser.add_argument(
        "--cf_model",
        nargs="?",
    ...
    parser.add_argument(
        "--regs", nargs="?", default="[1e-5,1e-5,1e-2]", help="for emb_loss."
    )  # default: '[1e-5,1e-5,1e-2]'
    parser.add_argument("--lr", type=float, default=0.0055, help="Learning rate.")
    parser.add_argument(
        "--emm", default=1e-3, type=float, help="for feature embedding bpr"
    )  #
    ...
    parser.add_argument(
        "--model_cat_rate", type=float, default=0.55, help="model_cat_rate"
    )
    ...
    parser.add_argument(
        "--real_data_tau", default=0.005, type=float, help="for real_data soft"
    )  #
    ...
    parser.add_argument(
        "--m_topk_rate", default=0.0001, type=float, help="for reconstruct"
    )
    parser.add_argument(
        "--log_log_scale", default=0.00001, type=int, help="log_log_scale"
    )
